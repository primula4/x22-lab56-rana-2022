...
document.getElementById("task-submit").addEventListener("click", async () => {
    ...
    let taskCityEl = document.getElementById("task-city");
    let taskCountryEl = document.getElementById("task-country");
    ...
    let taskCity = taskCityEl.value;
    let taskCountry = taskCountryEl.value;
    let lati;
    let lngi;
    ...
    // console.log(lngi);
    if (taskName.trim() != '') {
        let newTask = { name: taskName, address: taskAddress, postcode: taskPostcode,city: taskCity,country: taskCountry, streetNo: taskStreetNo, lat: lati, lng: lngi };
